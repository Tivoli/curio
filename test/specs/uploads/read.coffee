...
describe 'Uploads GET show', ->
  ...
  it 'should get the default image with no params', (done) ->
    stream  = request.get('http://127.0.0.1:3001/uploads/images/image')
  ...
  it 'should get a resized image by width', (done) ->
    stream  = request.get('http://127.0.0.1:3001/uploads/images/image?w=100')
    gm(stream).size (err, size) ->
  ...
  it 'should get a resized image by height', (done) ->
    stream  = request.get('http://127.0.0.1:3001/uploads/images/image?h=100')
    gm(stream).size (err, size) ->
  ...
  it 'should get a cropped and resized image', (done) ->
    stream  = request.get('http://127.0.0.1:3001/uploads/images/image?w=50&h=50&m=crop')
    gm(stream).size (err, size) ->
